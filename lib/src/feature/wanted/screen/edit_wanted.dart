...
class EditWanted extends StatefulWidget {
  final WantedModel wantedModel;
  const EditWanted({required this.wantedModel});

  @override
  _EditWantedState createState() => _EditWantedState();
}

class _EditWantedState extends State<EditWanted> {
  ...
  String? project;
  String? market;
  PropertyTypeModel? typeId;
  MarketTypeModel? maId;
  String? proId;
  String? ketId;

  void initState() {
    _desCtrl.text = widget.wantedModel.description;
    _minCtrl.text = widget.wantedModel.mixPrice;
    _maxCtrl.text = widget.wantedModel.maxPrice;
    _contactCtrl.text = widget.wantedModel.contactNumber;
    propertyType = widget.wantedModel.propertyTypeModel.name;
    market = widget.wantedModel.marketTypeModel.name;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: standardAppBar(context, "Edit Wanted"),
      body: Builder(
        builder: (context) {
        ...
        child: FlatButton(
            ...
            onPressed: () {
              if (_formKey!.currentState!.validate()) {
                if (propertyType == _protypeCtrl.text &&
                    market == _marketCtrl.text) {
                  proId = widget.wantedModel.propertyTypeModel.id;
                  ketId = widget.wantedModel.marketTypeModel.id;
                  BlocProvider.of<WantedBloc>(context).add(UpdateWantedStarted(
                      id: widget.wantedModel.id,
                      description: _desCtrl.text,
                      propertyTypeId: widget.wantedModel.propertyTypeModel.id!,
                      minPrice: _minCtrl.text,
                      maxPrice: _maxCtrl.text,
                      marketTypeId: widget.wantedModel.marketTypeModel.id!,
                      contactNumber: _contactCtrl.text));
                } else {
                  if (propertyType != _protypeCtrl.text &&
                      market == _marketCtrl.text) {
                    PropertyTypeModel protypeId =
                        BlocProvider.of<PropertyTypeBloc>(context)
                            .propertytypelist
                            .firstWhere(
                                (element) => element.name == _protypeCtrl.text);
                    // proId = typeId!.id;
                    print("proId $protypeId");
                    // ketId = widget.wantedModel.marketTypeModel.id;
                    BlocProvider.of<WantedBloc>(context).add(
                        UpdateWantedStarted(
                            id: widget.wantedModel.id,
                            description: _desCtrl.text,
                            propertyTypeId: protypeId.id!,
                            minPrice: _minCtrl.text,
                            maxPrice: _maxCtrl.text,
                            marketTypeId:
                                widget.wantedModel.marketTypeModel.id!,
                            contactNumber: _contactCtrl.text));
                  } else if (market != _marketCtrl.text &&
                      propertyType == _protypeCtrl.text) {
                    MarketTypeModel mymarket =
                        BlocProvider.of<MarketTypeBloc>(context)
                            .makert
                            .firstWhere(
                                (element) => element.name == _marketCtrl.text);
                     print("makeert $mymarket");
                    // ketId = maId!.id;
                    // print(ketId);
                    // proId = widget.wantedModel.propertyTypeModel.id;
                    BlocProvider.of<WantedBloc>(context).add(
                        UpdateWantedStarted(
                            ...
                            description: _desCtrl.text,
                            propertyTypeId:
                                widget.wantedModel.propertyTypeModel.id!,
                            minPrice: _minCtrl.text,
                            maxPrice: _maxCtrl.text,
                            marketTypeId: mymarket.id!,
                            contactNumber: _contactCtrl.text));
                  }
                }
              }
